...
module IrOperations( Deps :
        sig
            ...
            type value
            type typ
            val lift_core_typ_to_typ : typ Types.core_typ -> typ
            val unlift_typ_to_core_typ : typ -> typ Types.core_typ option
            val lift_core_value_to_value : (term, typ) core_value -> value
            val unlift_value_to_core_value : value -> (term, typ) core_value option
        end [@inject] ) =
struct
  open Deps
  ...
  let core_typecheck
    ...
    let utc x =  unlift_typ_to_core_typ  (typecheck env x) in
    let tc x = (typecheck env x) in
    match term with
    | IntE _ -> lift_core_typ_to_typ IntT
    | BoolE _ -> lift_core_typ_to_typ BoolT
    | UnitE -> lift_core_typ_to_typ UnitT
    | StringE _ -> lift_core_typ_to_typ StringT
    | LamE (v, vt, b) ->
        let env' = (v, vt) :: env in
        let bt = typecheck env' b in
        lift_core_typ_to_typ (Arrow (vt, bt))
    | PlusE (l, r) -> (
      match (utc l, utc r) with
      | Some IntT, Some IntT -> lift_core_typ_to_typ IntT
  ...
  let core_eval
    ...
    let ue x = unlift_value_to_core_value (eval env x) in
    let e x = eval env x in
    match term with
    | IntE i -> lift_core_value_to_value (IntV i)
    | UnitE -> lift_core_value_to_value UnitV
    | BoolE b -> lift_core_value_to_value (BoolV b)
    | StringE s -> lift_core_value_to_value (StringV s)
    | LamE (v, t, b) -> lift_core_value_to_value (LamV (v, t, b))
    | PlusE (t1, t2) -> ( match (ue t1, ue t2) with Some (IntV i1), Some (IntV i2) -> lift_core_value_to_value (IntV (i1 + i2)) | _ -> failwith "eval fail" )
